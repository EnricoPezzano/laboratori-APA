...
bool consensus(vector<vector<int> > gen){
	int acc = 0;
	int s = 0;
	for (int j = 0; j < 3 ; j++){
		if(gen[j][s]==1)
...
void start(vector<vector<int> > &gen){
	int z = 0;
	for (int j = 0; j < 3 ; j++){
		if(j==0){
			for(int i = 0; i < 3; i++){
...
// Ad ogni round i processi affidabili prendono una decisione in base al tally e alla maggioranza; il processo malevolo dice il contrario
void round(vector<vector<int> > &gen){
	...
	for (int j = 0; j < 3 ; j++){
		if(j==0){
			for(int i = 0; i < 3; i++){
				if(compute_tally(compute_maj(gen[s]), gen[s])>=3)  // if tally(i) ≥ 2t + 1
					gen[0][i] = compute_maj(gen[s]);   // then b(i) <- maj(i)
				else 
		...
		gen[j] = gen[0]; // i 2t + 1 processi affidabili switchano le scelte
		gen[j][3] = 1-gen[j][z];  // il processo maligno sceglie ad ogni round il contrario della dcisione di un processo affidabile
		z++;
	}
}
...
int probAcc (vector<int>numeriRound){
	...
	for (int i = 0; i<numeriRound.size(); i++){  // riempo un vettore con tutti i possibili numeri di round dopo i quali si raggiunge un accordo e conto quate volte escono
  	...
  	for (int i = 0; i<ripetitions.size(); i++){  // sommo le volte in cui si raggiunge un accordo in tali round e se questo supera il 99.9% delle volte lo ritorno
    	somma+=ripetitions[i];
    	if (somma>=ITERATIONS-ITERATIONS*0.001)
    		return i;
  	}
  	return 0;
}
...
int main()
{
	...
	for (int i = 0; i < ITERATIONS; i++){
		nRound=0;
		start(gen); // genero randomicamente le decisioni iniziali dei processi affidabili
		while (!consensus(gen)){  // finchè non si raggiunge un accordo si va avanti con i round
			round(gen);
			nRound++;
		}
	...
	cout<<"\nNumero di round dopo il quale la probabilità che l'accordo è raggiunto è più grande del 99.9%: "<<probAcc(numeriRound)<<endl;
	
	// calcolo la media
	int somma = 0;
	for (int i = 0; i < numeriRound.size(); i++)
		somma += numeriRound[i];
	float media = (float)somma/ITERATIONS;
	...
	float mediaPow = (float)somma/ITERATIONS;
