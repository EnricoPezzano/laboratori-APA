// https://2021.aulaweb.unige.it/mod/assign/view.php?id=108625
...
int accprob (int a[r]){
	// Determino il numero di round dopo il quale la probabilità che l'accordo abbia raggiunto il più grande del 99.9%
	int ripetizioni[r];
	int somma = 0;
...
int main()
{
   srand((unsigned)time(NULL));
	...
	for(int l=0;l<r;l++){
			...
			// itero con una variabile i sulle righe della matrice
				for(int i=0; i<3 ;i++){
					...
					cout<<endl<<endl<<"MAJ:       "<<maj<<"    TALLY:         "<<tally<<"          MONETA:           "<<moneta<<endl<<endl;
						 // Se tally > 2t+1
					if(tally>=3){
							a2[i]=maj; // Nel prossimo round il processo i avr� valore = maj, quindi tutta l'i-esima colonna della matrice avr� solo il valore maj
					}
					else{// altrimenti

						a2[i]=moneta;// Nel prossimo round il processo i avr� valore = moneta, quindi tutta l'i-esima colonna della matrice avr� solo il valore moneta	
      ...
      //calcolo la varianza usando la formula E[u^2] - �^2
